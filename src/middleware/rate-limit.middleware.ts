...
const checkRateLimit = async (key: string, config: RateLimitConfig): Promise<RateLimitInfo> => {
  ...
  try {
    ...
    console.error(colors.red('❌ Rate limit check failed:'), error);
    // Fail-open policy
    return {
      totalRequests: 0,
...
export const createRateLimiter = (prefix: string, config: RateLimitConfig) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      console.error(colors.red('❌ Rate limiter error:'), error);
      next(); // Fail-open
    }
  };
};
...
export const resetRateLimit = async (identifier: string, prefix: string): Promise<void> => {
  try {
    ...
    console.log(colors.blue(`🔄 Rate limit reset: ${key}`));
  } catch (error: any) {
    console.error(colors.red('❌ Failed to reset rate limit:'), error);
  }
};
...
export const getRateLimitInfo = async (
  ...
  try {
    ...
    console.error(colors.red('❌ Failed to get rate limit info:'), error);
    return null;
  }
};
