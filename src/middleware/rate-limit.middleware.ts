...
const checkRateLimit = async (key: string, config: RateLimitConfig: Promise<RateLimitInfo> => {
  ...
  try {
    ...
    const exists = await RedisService.existsCache(key;
    if (current === 1 || !exists {
      await RedisService.updateTTL(key, expirySeconds;
    }
    let ttl = await RedisService.getTTL(key;
    if (ttl === -1 {
      await RedisService.updateTTL(key, expirySeconds;
      ttl = expirySeconds;
    }
    ...
    if (current > config.maxRequests {
      logger.error(
        `🚨 Rate limit exceeded: ${key} (${current}/${config.maxRequests}`
      ;
    }
...
: Promise<RateLimitInfo | null> => {
  try {
    ...
    const requestsStr = await RedisService.getCache<string>(key;
